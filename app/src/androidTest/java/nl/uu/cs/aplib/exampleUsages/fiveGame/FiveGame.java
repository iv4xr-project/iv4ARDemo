...
public class FiveGame {
    ...
    static public class RandomPlayer {
         ...
         */
        public boolean move() {
            LinkedList<int[]> empties = new LinkedList<int[]>();
            LinkedList<int[]> emptiesAdjacent = new LinkedList<int[]>();
            ...
            if (!emptiesAdjacent.isEmpty()) {
                int[] position = emptiesAdjacent.get(rnd.nextInt(emptiesAdjacent.size()));
                game.move(ty, position[0], position[1]);
                return true;
            }
            if (!empties.isEmpty()) {
                int[] position = empties.get(rnd.nextInt(empties.size()));
                game.move(ty, position[0], position[1]);
                return true;
            }
            return false;
