...
public class DumbDoctor {
    ...
    static public void main(String[] args) {
        ...
        Tactic.PrimitiveTactic opening = action("opening").desc("To say an opening question to the patient.")
                .do1((DoctorBelief belief) -> {
                    belief.env().ask("How do you feel today?");
                    return ++belief.patientHappiness;
                }).lift();

        Tactic.PrimitiveTactic a1 = action("a1").desc("To ask a 'smart' question to the patient :D").do1((DoctorBelief belief) -> {
            belief.env().ask("Please explain a bit more...");
            return ++belief.patientHappiness;
        }).lift();

        Tactic.PrimitiveTactic a2 = action("a2").desc("Another 'smart' question to pose to the patient ;)").do1((DoctorBelief belief) -> {
        ...
        DoctorBelief belief = new DoctorBelief();
        belief.setEnvironment(new ConsoleEnvironment());
        BasicAgent doctorAgent = new BasicAgent().attachState(belief).setGoal(topgoal);
