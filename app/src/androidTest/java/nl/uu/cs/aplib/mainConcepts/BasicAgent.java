...
public class BasicAgent {
    ...
    public void addAfter(GoalStructure G) {
        ...
        GoalStructure parent = currentGoal.parent;
        if (parent.combinator == GoalsCombinator.REPEAT) {
            ...
            GoalStructure H = SEQ(currentGoal, G);
    ...
    public void addBefore(GoalStructure G) {
        ...
        GoalStructure g1 = SEQ(G, currentGoal);
        GoalStructure repeatNode = REPEAT(g1);
    ...
    private boolean removeGoalWorker(GoalStructure H, GoalStructure tobeRemoved) {
        if (H.subgoals.contains(tobeRemoved)) {
            ...
            for (GoalStructure H2 : H.subgoals) {
                boolean r = removeGoalWorker(H2, tobeRemoved);
                if (r)
                    return true;
            }
            return false;
    ...
    private void updateWorker() {
        ...
        List<Tactic.PrimitiveTactic> candidates = currentTactic.getFirstEnabledActions(state);
        ...
        // to decide:
        Tactic.PrimitiveTactic chosenAction = deliberation.deliberate(state, candidates);
        ...
        long elapsed = mytime.elapsedTimeSinceLastSample();
