...
public class FiveGame_withAgent {
    ...
    static class MyState extends State {
        ...
        public MyState setEnvironment(Environment env) {
            ...
            // resolved yet
            PrologReasoner.Rule ruleWinWest = rule(winningMove.on("X", "Y")).impBy(eastNeighbor.on(cross, "A", "B", "Y"))
            ...
            PrologReasoner.Rule ruleWinEast = rule(winningMove.on("X", "Y")).impBy(occupied.on(cross, "A", "Y"))
            ...
            PrologReasoner.Rule ruleWinSouth = rule(winningMove.on("X", "Y")).impBy(northNeighbor.on(cross, "X", "A", "B"))
            ...
            PrologReasoner.Rule ruleWinNorth = rule(winningMove.on("X", "Y")).impBy(occupied.on(cross, "X", "A"))
            ...
            PrologReasoner.Rule ruleBlock4WestEast = rule(blockMove.on("X", "Y")).impBy(eastNeighbor.on(circle, "A", "B", "Y"))
            ...
            PrologReasoner.Rule ruleBlock4SouthNorth = rule(blockMove.on("X", "Y")).impBy(northNeighbor.on(circle, "X", "A", "B"))
            ...
            PrologReasoner.Rule rule4WestEast = rule(set4Move.on("X", "Y")).impBy(eastNeighbor.on(cross, "A", "B", "Y"))
                    .and(eastNeighbor.on(cross, "B", "C", "Y")).and(eastNeighbor.on(cross, "C", "D", "Y"))
            ...
            PrologReasoner.Rule rule4SouthNorth = rule(set4Move.on("X", "Y")).impBy(northNeighbor.on(cross, "X", "A", "B"))
                    .and(northNeighbor.on(cross, "X", "B", "C")).and(northNeighbor.on(cross, "X", "C", "D"))
        ...
        public void updateState(String agentID) {
        	Pair<FiveGame.GAMESTATUS, FiveGame.SQUARE[][]> obs = env().observe(agentID) ;
        	board = obs.snd ;
        }
        ...
        List<Square_> getEmptySquares() {
            LinkedList<Square_> r = new LinkedList<Square_>();
            for (int x = 0; x < boardsize; x++)
                for (int y = 0; y < boardsize; y++) {
    ...
    static private void test() throws InvalidTheoryException {
        int N = 8;
        FiveGame thegame = new FiveGame(N, 0);
        // create an agent state and an environment, attached to the game:
        MyState state = new MyState().setEnvironment(new FiveGameEnv().attachGame(thegame));
        ...
        PrologReasoner.QueryResult solution = state.prolog().query("set4Move(X,Y)");
    ...
    static public void main(String[] args) throws InvalidTheoryException, NoSolutionException, MalformedGoalException {
        ...
        FiveGame thegame = new FiveGame(7, 0);
        // create an agent state and an environment, attached to the game:
        MyState state = new MyState().setEnvironment(new FiveGameEnv().attachGame(thegame));
        // creatint the agent:
        BasicAgent agent = new BasicAgent().attachState(state);
        ...
        Tactic.PrimitiveTactic besideHV = action("besideHV").do1((MyState st) -> {
            List<Square_> empties = st.getEmptySquaresWithHVNeighboringCross();
            if (empties.isEmpty())
                empties = st.getEmptySquares();
            if (empties.isEmpty())
                return null;
            Square_ sq = empties.get(rnd.nextInt(empties.size()));
            FiveGame.GAMESTATUS status = st.env().move(SQUARE.CROSS, sq.x, sq.y);
            st.markMove(SQUARE.CROSS, sq.x, sq.y);
            return status;
        }).lift();

        // put a cross if there is a winning horizontal or vertical conf:
        Tactic.PrimitiveTactic winningmove = action("winningmove").do2((MyState st) -> (QueryResult qsolution) -> {
            if (qsolution == null)
                return null;
            int x = qsolution.int_("X");
            int y = qsolution.int_("Y");
            FiveGame.GAMESTATUS status = st.env().move(SQUARE.CROSS, x, y);
        ...
        Tactic.PrimitiveTactic block = action("block").do2((MyState st) -> (QueryResult qsolution) -> {
            if (qsolution == null)
                return null;
            int x = qsolution.int_("X");
            int y = qsolution.int_("Y");
        ...
        Tactic.PrimitiveTactic smartmove = action("smartmove").do2((MyState st) -> (QueryResult qsolution) -> {
            if (qsolution == null)
                return null;
            int x = qsolution.int_("X");
            int y = qsolution.int_("Y");
        ...
        GoalStructure.PrimitiveGoal g = goal("goal").toSolve((GAMESTATUS st) -> st == GAMESTATUS.CROSSWON)
                .withTactic(FIRSTof(winningmove, block, smartmove, besideHV)).lift();

        agent.setGoal(g);
        FiveGame.RandomPlayer opponent = new FiveGame.RandomPlayer(SQUARE.CIRCLE, thegame);
