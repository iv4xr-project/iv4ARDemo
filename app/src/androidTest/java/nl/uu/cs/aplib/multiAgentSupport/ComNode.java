...
public class ComNode {
    ...
    synchronized public void register(AutonomousBasicAgent agent) {
        ...
        Set<nl.uu.cs.aplib.agents.AutonomousBasicAgent> brothers = roleMap.get(role);
        if (brothers == null) {
    ...
    synchronized public void deregister(AutonomousBasicAgent agent) {
        idMap.remove(agent);
        String role = agent.getRole();
        Set<AutonomousBasicAgent> brothers = roleMap.get(role);
        if (brothers == null)
            return;
    ...
    public Acknowledgement send(Message msg) {
        ...
        AutonomousBasicAgent sender = idMap.get(senderId);
        if (sender == null) {
            // unknown sender!
        ...
        switch (msg.castTy) {
        case SINGLECAST:
            AutonomousBasicAgent receiver = idMap.get(msg.idTarget);
            if (receiver == null) {
        ...
        case BROADCAST:
            ...
            return new Acknowledgement(AckType.SUCCESS, null);
        case ROLECAST:
            Set<AutonomousBasicAgent> receivers = roleMap.get(msg.idTarget);
            if (receivers != null) {
