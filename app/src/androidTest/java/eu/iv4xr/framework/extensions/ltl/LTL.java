...
public abstract class LTL<State> extends SequencePredicate<State> {
    ...
    public WeakUntil<State> weakUntil(LTL<State> psi) {
        WeakUntil ltl = new WeakUntil<State>();
        ltl.phi1 = this.treeClone() ;
        ltl.phi2 = psi.treeClone() ;
        return ltl ;
    }
    ...
    public static class Not<State> extends LTL<State> {
        ...
        public SATVerdict sat() {
            ...
            phi.sat();
            Iterator<LTL.LTLVerdictInfo> iterator = evals.descendingIterator();
            Iterator<LTL.LTLVerdictInfo> iteratorPhi = phi.evals.descendingIterator();

            while (iterator.hasNext()) {
                LTL.LTLVerdictInfo psi = iterator.next();
                SATVerdict p = iteratorPhi.next().verdict;
                switch (p) {
                    case SAT:
    ...
    public static class And<State> extends LTL<State> {
        ...
        public SATVerdict sat() {
            ...
            while (iterator.hasNext()) {
                ...
                for (int k = 0; k < N; k++) {
                    SATVerdict p = conjuntIterators[k].next().verdict;
    ...
    public static class Or<State> extends LTL<State> {
        ...
        public SATVerdict sat() {
            ...
            while (iterator.hasNext()) {
                ...
                for (int k = 0; k < N; k++) {
                    SATVerdict p = disjunctIterators[k].next().verdict;
    ...
    public static class Until<State> extends LTL<State> {
        ...
        public SATVerdict sat() {
            ...
            phi2.sat();
            Iterator<LTL.LTLVerdictInfo> iterator = evals.descendingIterator();
            Iterator<LTL.LTLVerdictInfo> iteratorPhi1 = phi1.evals.descendingIterator();
            Iterator<LTL.LTLVerdictInfo> iteratorPhi2 = phi2.evals.descendingIterator();
            ...
            while (iterator.hasNext()) {
                LTL.LTLVerdictInfo psi = iterator.next();
                SATVerdict p = iteratorPhi1.next().verdict;
                SATVerdict q = iteratorPhi2.next().verdict;
                if (q == SATVerdict.SAT) {
                    psi.verdict = SATVerdict.SAT;
                    nextSat = true;
                } else {
        ...
        public Until<State> treeClone() {
            Until<State> clone = new Until<State>() ;
    ...
    public static class WeakUntil<State> extends LTL<State> {
        ...
        public WeakUntil<State> treeClone() {
            WeakUntil<State> clone = new WeakUntil<State>() ;
    ...
    public static class Next<State> extends LTL<State> {
        ...
        public SATVerdict sat() {
            ...
            while (iterator.hasNext()) {
                psi = iterator.next();
                SATVerdict q = iteratorPhi.next().verdict;
                switch (q) {
                    case SAT:
                        psi.verdict = SATVerdict.SAT;
                        break;
        ...
        public Next<State> treeClone() {
            Next<State> clone = new Next<State>() ;
            clone.phi = this.phi.treeClone() ;
            return clone ;
        }
...
import eu.iv4xr.framework.extensions.ltl.SequencePredicate;
