...
public class BuchiModelChecker {
	 ...
	 * terminate if the target program has an infinite state space.
	 */
	...
	public SATVerdict sat(Buchi buchi, int maxDepth) {
		BasicModelChecker.Path<Pair<IExplorableState, String>> path = find(buchi,maxDepth) ;
	...
	public Path<Pair<IExplorableState,String>> find(Buchi buchi, int maxDepth) {
		...
		List<Pair<Buchi.BuchiTransition, Integer>> initialEnabledBuchiTransitions = buchi.getEnabledTransitions(targetInitialState) ;
		if (initialEnabledBuchiTransitions.isEmpty()) return null ;
		
		// quantify over all possible initial transitions of Buchi:
		for(Pair<Buchi.BuchiTransition, Integer> tr0 : initialEnabledBuchiTransitions) {
			...
			if (witnessPath != null) {
				...
				for(Pair<ITransition, Pair<IExplorableState, Integer>> step : witnessPath.path) {
	...
	{	
		...
		List<ITransition> enabledModelTransitions = model.availableTransitions() ;
		
		for(ITransition trModel : enabledModelTransitions) {
			
			model.execute(trModel);
			
			IExplorableState modelNextState = model.getCurrentState().clone() ;
			List<Pair<Buchi.BuchiTransition, Integer>> enabledBuchiTransitions = buchi.getEnabledTransitions(modelNextState) ;
			
			for (Pair<Buchi.BuchiTransition, Integer> trBuchi : enabledBuchiTransitions) {
				...
				if(buchi.omegaAcceptingStates.contains(buchiCurrentState)) // yes, give it the state before the combinedNextState
				{
					...
					Path<Pair<IExplorableState,Integer>> path = dfsBuchi(buchi,
							pathSoFar,
							statesInPathSoFar, 
							freshVisitedStates, 
							combinedNextState, 
