...
public class BasicModelChecker {
	...
	public static class Path<State> {
		...
		Path<State> copy() {
			...
			for(Pair<ITransition, State> step : this.path) {
		...
		public State getLastState() {
			List<State> seq = getStateSequence() ;
		...
		public String toString() {
			int k = 0;
			String s = "";
			for (Pair<ITransition, State> tr : path) {
				if (k > 0)
					s += "\n";
	...
	public SATVerdict sat(Predicate<IExplorableState> q) {
		BasicModelChecker.Path<IExplorableState> path = find(q,Integer.MAX_VALUE) ;
	...
	public SATVerdict sat(Predicate<IExplorableState> q, int maxDepth) {
		BasicModelChecker.Path<IExplorableState> path = find(q,maxDepth) ;
		if(path == null) return SATVerdict.UNSAT ;
		return SATVerdict.SAT ;
	}
	...
	Path<IExplorableState> dfs(Predicate<IExplorableState> whatToFind,
		...
		List<ITransition> nextTransitions = model.availableTransitions() ;
		for(ITransition tr: nextTransitions) {
			model.execute(tr);
			IExplorableState nextState = (IExplorableState) model.getCurrentState().clone() ;
	...
	public <CoverageItem> TestSuite<CoverageItem> testSuite(
		...
		while (! worklist.isEmpty()) {
			CoverageItem target = worklist.remove(0) ;
