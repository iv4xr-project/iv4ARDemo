...
public class Buchi {
	...
	public Buchi withTransition(String from, 
		...
		BuchiTransition transition = new BuchiTransition() ;
		transition.id = transitionId ;
		transition.name = transitionName ;
		transition.condition = condition ;
		List<Pair<Buchi.BuchiTransition, Integer>> transitionGroup = transitions.get(fromId) ;
	...
	public int numberOfTransitions() {
		int n = 0 ;
		for (Collection<Pair<Buchi.BuchiTransition, Integer>> trgroup : transitions.values()) {
			n += trgroup.size() ;
		}
		return n ;
	}
	...
	public String toString() {
		...
		int k = 0 ;
		for(Map.Entry<String, Integer> st : states.entrySet()) {
			buf.append("\n   " + st.getValue() + ":") ;
		...
		for(Map.Entry<Integer, List<Pair<Buchi.BuchiTransition, Integer>>> trgroup : transitions.entrySet()) {
			String from = decoder[trgroup.getKey()] ;
			for (Pair<Buchi.BuchiTransition, Integer> tr : trgroup.getValue()) {
				String to = decoder[tr.snd] ;
				String tr_ = tr.fst.name ;
				buf.append("\n   " 
				   + tr.fst.id + ": "
				   + from + " ---" + tr_ + "--> " + to)  ;
			}
	...
	Buchi appendStateNames(String suffix) {
		Map<String, Integer> states__ = this.states ;
		this.states = new HashMap<String,Integer>() ;
		for(Map.Entry<String, Integer> st : states__.entrySet()) {
	...
	Buchi insertNewState(String st) {
		String[] oldDecoder = this.decoder ;
		...
		for (Map.Entry<String, Integer> st_ : this.states.entrySet()) {
		...
		Set<Integer> oldOmegaAcceptingStates = this.omegaAcceptingStates ;
		this.omegaAcceptingStates = new HashSet<>() ;
		for (Integer s : oldOmegaAcceptingStates) {
			this.omegaAcceptingStates.add(s+1) ;
		}
		Set<Integer> oldTraditionalAcceptingStates = this.traditionalAcceptingStates ;
		this.traditionalAcceptingStates = new HashSet<>() ;
		for (int s : oldTraditionalAcceptingStates) {
			this.traditionalAcceptingStates.add(s+1) ;
		}
		// adjust the transitions:
		Map<Integer, List<Pair<Buchi.BuchiTransition, Integer>>> oldTransitions = this.transitions;
		this.transitions = new HashMap<>() ;
		transitions.put(0, new LinkedList<Pair<BuchiTransition,Integer>>()) ;
		for (Map.Entry<Integer, List<Pair<Buchi.BuchiTransition, Integer>>> trgroup : oldTransitions.entrySet()) {
	...
	public List<Pair<BuchiTransition,Integer>> getEnabledTransitions(IExplorableState state) {
		...
		List<Pair<Buchi.BuchiTransition, Integer>> availableTransitions = transitions.get(currentState) ;
