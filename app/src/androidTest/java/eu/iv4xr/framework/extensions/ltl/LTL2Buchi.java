...
public class LTL2Buchi {
	...
	public static <State> LTL<State> pushNegations(LTL<State> f) {
		...
		if (f instanceof Next) {
			Next<State> f_ = (Next<State>) f ;
			f_.phi = pushNegations(f_.phi) ;
			return f_ ;
		}
		if (f instanceof And) {
			And<State> f_ = (And<State>) f ;
		...
		if (f instanceof WeakUntil) {
			WeakUntil<State> f_ = (WeakUntil<State>) f ;
			f_.phi1 = pushNegations(f_.phi1) ;
			f_.phi2 = pushNegations(f_.phi2) ;
			return f_ ;
		}
		...
		if(phi_case4 != null) {
			...
			LTL<State>[] inners = Arrays.asList(phi_case4.conjuncts)
			   .stream()
		...
		if(phi_case5 != null) {
			...
			LTL<State>[] inners = Arrays.asList(phi_case5.disjuncts)
				.stream()
				.map((LTL<State> g) -> pushNegations(ltlNot(g)))
				.collect(Collectors.toList())
				.toArray(dummy)
		...
		if(phi_case6 != null) {
			...
			LTL<State> psi1 = pushNegations(ltlAnd(phi_case6.phi1,
					                        ltlNot(phi_case6.phi2))) ;

			LTL<State> psi2 = pushNegations(ltlAnd(ltlNot(phi_case6.phi1),
                                            ltlNot(phi_case6.phi2))) ;
		...
		if(phi_case7 != null) {
			...
			LTL<State> psi1 = pushNegations(ltlAnd(phi_case7.phi1,
					                    ltlNot(phi_case7.phi2))) ;

			LTL<State> psi2 = pushNegations(ltlAnd(ltlNot(phi_case7.phi1),
                                        ltlNot(phi_case7.phi2))) ;
	...
	private static Pair<Buchi,Integer> getBuchiFromUntil(int buchiNr, Until<IExplorableState> phi) {
		...
		Pair<Buchi, Integer> rec = getBuchiWorker(buchiNr+1, phi.phi2) ;
		...
		String t0Id = "t_" + S + "_" + S ;
		BF.withTransition(S,S,t0Id,atomName(phi.phi1),atomPred(phi.phi1)) ;
		
		// transitions that branch out from BF's original init:
		List<Pair<Buchi.BuchiTransition, Integer>> initArrowsOut = BF.transitions.get(oldInitialState) ;
		int tid = 1 ;
		for(Pair<Buchi.BuchiTransition, Integer> tr : initArrowsOut) {
			Buchi.BuchiTransition tr_ = tr.fst ;
	...
	private static Pair<Buchi,Integer> getBuchiFromNext(int buchiNr, Next<IExplorableState> phi) {
		...
		Pair<Buchi, Integer> rec = getBuchiWorker(buchiNr+1, phi.phi) ;
		Buchi BF = rec.fst.treeClone() ;
		int nextbuchiNr = rec.snd ;
		...
		BF.initialState = BF.states.get(S) ;
		String tId = "t_" + S + "_" +  oldInitialStateName ;
	...
	private static Buchi union(Buchi B1, Buchi B2) {
		...
		for(int f : B1.traditionalAcceptingStates) {
			String fname = B1.decoder[f] ;
			int f_newindex = B2_.states.get(fname) ;
			B2_.traditionalAcceptingStates.add(f_newindex) ;
		}
		
		for(int f : B1.omegaAcceptingStates) {
			String fname = B1.decoder[f] ;
			int f_newindex = B2_.states.get(fname) ;
			B2_.omegaAcceptingStates.add(f_newindex) ;
		}
		
		Buchi B1_ = B1.treeClone() ;
		for (Map.Entry<Integer, List<Pair<Buchi.BuchiTransition, Integer>>> tr : B1_.transitions.entrySet()) {
		...
		List<Pair<Buchi.BuchiTransition, Integer>> B2initArrowsOut = B2_.transitions.get(B2initState_newIndex) ;
		// need some number to assign to the ids of the new transitions...
		int tid = B1.transitions.get(B1.initialState).size() ;
		for(Pair<Buchi.BuchiTransition, Integer> tr : B2initArrowsOut) {
			Buchi.BuchiTransition tr_ = tr.fst ;
	...
	private static Pair<Buchi,Integer> getBuchiFromOr(int buchiNr, Or<IExplorableState> phi) {
		...
		Pair<Buchi, Integer> rec = getBuchiWorker(buchiNr, phi.disjuncts[0]) ;
	...
	private static Pair<Buchi,Integer> getBuchiFromAnd(int buchiNr, And<IExplorableState> phi) {
		...
		Pair<Buchi, Integer> rec = getBuchiWorker(buchiNr+1,q) ;
		Buchi BF = rec.fst.treeClone() ;
		int nextbuchiNr = rec.snd ;		
