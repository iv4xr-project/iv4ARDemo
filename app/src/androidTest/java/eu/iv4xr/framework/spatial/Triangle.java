...
public class Triangle implements LineIntersectable {
    ...
    public Collection<Vec3> intersect(Line l) {
        ...
        ArrayList<Vec3> result = new ArrayList<Vec3>(2);
        Vec3 e1 = Vec3.sub(B, A);
        Vec3 e2 = Vec3.sub(C, A);
        Vec3 n = Vec3.cross(e1, e2);
        float det = -Vec3.dot(l.direction(), n);
        float idet = 1.0f / det;
        Vec3 ao = Vec3.sub(l.origin(), A);
        Vec3 dao = Vec3.cross(ao, l.direction());
        float u = Vec3.dot(e2, dao) * idet;
        float v = -Vec3.dot(e1, dao) * idet;
        float t = Vec3.dot(ao, n) * idet;
