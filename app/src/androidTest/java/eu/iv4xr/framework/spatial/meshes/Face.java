...
public class Face implements Iterable<Integer> {
    ...
    public float distFromPoint(Vec3 w, ArrayList<Vec3> concreteVertices) {
        ...
        Vec3 vert_0 = concreteVertices.get(vertices[0]);

        Vec3 line_0_to_1 = Vec3.sub(concreteVertices.get(vertices[1]), vert_0);
        Vec3 line_lastNode_to_0 = Vec3.sub(vert_0, concreteVertices.get(vertices[N - 1]));
        // the normal vector between the above two lines, which is also parallel with
        // the
        // face normal vector:
        Vec3 norm = Vec3.cross(line_0_to_1, line_lastNode_to_0);
        ...
        for (int i = 0; i < N; i++) {
            Vec3 p = concreteVertices.get(vertices[i]);
            // System.out.println("### v" + i + " " + p) ;
            Vec3 p_next = concreteVertices.get(vertices[(i + 1) % N]);
            Vec3 line_next_to_p = Vec3.sub(p_next, p);
            Vec3 line_w_to_p = Vec3.sub(w, p);
            float test = Vec3.dot(Vec3.cross(line_next_to_p, norm), line_w_to_p);
        ...
        if (inside_extruded_prism) {
            float z = Vec3.dot(Vec3.sub(w, vert_0), norm);
    ...
    public static void main(String[] args) {
        ...
        Vec3 v1 = new Vec3(2, 0, 0);
        Vec3 v2 = new Vec3(0, 2, 2);
