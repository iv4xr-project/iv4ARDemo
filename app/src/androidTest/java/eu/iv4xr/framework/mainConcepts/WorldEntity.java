...
public class WorldEntity implements Serializable {
    ...
    public boolean hasSameState(WorldEntity old) {
        ...
        if (!(equal_(position, old.position) && equal_(velocity, old.velocity)
            ...
            return false;
        for (Map.Entry<String, Serializable> P : properties.entrySet()) {
            Serializable q = old.properties.get(P.getKey());
            Serializable Pval = P.getValue() ;
        ...
        if (this.elements.size() != old.elements.size())
            return false;
        for (Map.Entry<String, WorldEntity> elem_ : elements.entrySet()) {
            WorldEntity elem2 = old.elements.get(elem_.getKey());
            if (elem2 == null)
                return false;
            WorldEntity elem1 = elem_.getValue();
    ...
    public boolean getBooleanProperty(String propertyName) {
        Serializable V = getProperty(propertyName);
        if (V == null)
            return false;
    ...
    public void assignTimeStamp(long ts) {
        timestamp = ts;
        for (WorldEntity e : elements.values())
    ...
    public WorldEntity deepclone() throws IOException, ClassNotFoundException {
        ...
        WorldEntity copied = (WorldEntity) in.readObject();
        return copied;
    }
