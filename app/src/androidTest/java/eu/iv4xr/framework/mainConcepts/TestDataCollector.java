...
public class TestDataCollector implements Parsable {
	...
	public void save(String fname, String covFname) throws IOException {
		...
		if (fname != null) {
			...
			for (String A : agents) {
				List<ObservationEvent.TimeStampedObservationEvent> trace = getTestAgentTrace(A);
				for (ObservationEvent.TimeStampedObservationEvent event : trace) {
	 ...
	 */
	public void reset() {
		for (EventTrace tr : this.perAgentEventTrace.values()) {
			tr.reset(); 
		}
		for (CoverageMap cmap : this.perAgentCoverage.values()) {
			cmap.reset(); 
		}
