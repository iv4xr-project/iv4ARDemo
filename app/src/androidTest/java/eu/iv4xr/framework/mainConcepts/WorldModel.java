...
public class WorldModel implements Serializable {
    ...
    public WorldEntity updateEntity(WorldEntity e) {
        ...
        WorldEntity current = elements.get(e.id);
        if (current == null) {
            // e is new:
            elements.put(e.id, e);
            return e;
            ...
            if (e.timestamp >= current.timestamp) {
                ...
                if (e.hasSameState(current)) {
                    long startTimeStutter = current.lastStutterTimestamp;
                ...
                WorldEntity prev = current.getPreviousState();
     ...
     * 
     * IMPORTANT: do not use this to merge WorldModel from other agents. Also note if an agent want to send
     * information about an entity it knows to share it with another agent, it
     * should send a copy of the entity may have side effect
     * on the entity.
     * 
    ...
    public List<WorldEntity> mergeNewObservation(WorldModel observation) {
        ...
        for (WorldEntity e : observation.elements.values()) {
            WorldEntity f = this.updateEntity(e);
            if (e == f) {
                // System.out.println("%%% updating " + e.id) ;
